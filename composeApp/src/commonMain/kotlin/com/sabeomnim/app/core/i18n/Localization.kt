...
object AppStrings {
    ...
    fun settingsVoice(lang: AppLanguage) = if (lang == AppLanguage.DANISH) "Udtalestemme" else "Voice"
    fun voiceFemale(lang: AppLanguage) = if (lang == AppLanguage.DANISH) "Kvindestemme" else "Female Voice"
    fun voiceFemaleDesc(lang: AppLanguage) = if (lang == AppLanguage.DANISH) "Klar og tydelig koreansk udtale" else "Clear and articulate Korean pronunciation"
    fun voiceMale(lang: AppLanguage) = if (lang == AppLanguage.DANISH) "Mandestemme (Sabeomnim)" else "Male Voice (Sabeomnim)"
    fun voiceMaleDesc(lang: AppLanguage) = if (lang == AppLanguage.DANISH) "Dyb og autoritær instruktørstemme" else "Deep and authoritative master voice"
